...
```{r read_in_output, echo=FALSE, warning=FALSE,message=FALSE}
library(here)
library(tidyverse)
#rm(list=ls())
# load(here("Model Fits","Hake_2019-23_Factor=2_ Bottle_trad_re =TRUE_ MESH (knots, cutoff) = (148,45) Hake 2024-01 TMB fit.RData"))
# invisible(list2env(Output,globalenv()))

```

## Supplement: Description of the statistical model.


Environmental DNA (eDNA, residual genetic material sampled from water, soil, or air) is an increasingly common tool for non-invasively sampling marine and aquatic ecological communities, valuable for estimating species occurrence [@veilleux2021], biodiversity [i.e., Richness @muenzel2024], and genetic structure [@andres2023a]. However, eDNA has not been widely used for quantitative abundance estimation [but see @shelton2022hake, @guri2024a, @stoeckle2024]. While the use of eDNA for quantification is supported both conceptually -- where there is more of a species, more cells are shed in the environment, and thus there is more DNA -- and empirically (e.g. there are strong linkages between single-species eDNA measures of in aquaria [@jo2019b; @ledger2024], rivers and streams [@pont2024], estuaries and coastal oceans [@dibattista2022; @baetscher2024; @maes2023; @shelton2019biocons], quantitative uses of eDNA observations have significantly lagged behind their use in occurrence and biodiversity applications. 

Here we use environmental DNA samples from three years of the U.S.-Canada Integrated Ecosystem \& Acoustic-Trawl Survey for Pacific hake to develop and index of abundance for a Pacific Hake. We incorporate samples collected over 13 degrees of latitude between the surface and 500m depth and spanning nearshore to the deep waters off the continental shelf. This effort is the largest and most extensive eDNA survey in the world and encompasses nearly 3000 sampling stations and nearly 6000 individual 2.5L water samples collected over the 3 survey years. In previous work, we showed that the distribution and abundance of hake DNA within a year (2019) largely mirrored the spatial distribution biomass of hake estimated from the acoustic trawl survey [@shelton2022hake]. Here, we extend our analyses to use hake DNA to describe changes in the hake population through time. To our knowledge this is the first use of eDNA to develop an index of abundance for stock assessment purposes.

### Connecting DNA and abundance metrics

Because this is the first use of an eDNA index in a stock assessment context, it is important to be describe the logic and assumptions that connect observations of DNA from water samples to measures of fish biomass or abundance.  The key difference between an eDNA survey and other traditional surveys is that traditional surveys depend on directly observing the organism of interest either through capture (e.g. via net sampling) or other means (e.g. via acoustic backscatter).  eDNA, in contrast, detects traces left behind by organisms -- cells derived from slime, scales, feces or other tissues -- not the organism itself.  A metaphor is that sampling eDNA is sampling the shadow of the organism. Therefore, the justification for using an eDNA index relies on understanding how such a shadow reflects the true population of interest. This relationship hinges on understanding two important processes: 1) the rate at which DNA is shed into the environment, and 2) the rate at which eDNA decays in aqueous environments. Recent reviews describe these processes in detail [e.g. @andruszkiewicz2021environmental; @Lamb2022MER; @Jo2023RSB] and we briefly describe some of the relevant literature here. Additional processes can affect eDNA [see e.g. @barnes2016ecology], but assumptions about shedding and decay predominate in this application.

Decay rates have been extensively studied in laboratory settings and show relatively rapid declines in DNA with most DNA decaying on the order or hours to a few days. [@sassoubre2016quantification; @andruszkiewicz2021environmental; @KirtaneEDNA2021]. While there are documentated effects of environmental conditions (e.g. temprature, pH) on eDNA decay, within the range of conditions within our sampling domain, assuming an invariant rates of decay among the sampling depths and locations is reasonable. That is, we assume any differences in decay between, for example, the far north and south of the domain or between shallow and deep water samples are relatively trivial.

Shedding rates are far more difficult to study than decay and while there are estimates of shedding across a wide range of fish species under different conditions [e.g. @sassoubre2016quantification; @Thalinger2021FrontEE; @jo2019effect], estimates vary wildly both within and among species. In our application which is focused on a single species (Pacific hake), the relevant question is whether shedding can be considered proportional to biomass .  There is a support from metabolic theory that shedding should scale in proportion to the surface area of individuals, not biomass [e.g. @YatesMEC2021]. Furthermore, metabolic theory would suggest that different ontogenetic stages or different age fish may have different shedding rates  (i.e. rapidly growing individuals may shed more DNA into the environment than slow growing individuals). To date, empirical evidence of differences among individuals of different ontogenetic stages or stages is equivocal [@Klymus2015; @Ostberg2022; @Wilder2023] and remains an active area of research. We note that both laboratory studies of related species [e.g. gadids, @ledger2024] and previous work on hake [@shelton2022hake] support a proportionality between hake biomass and eDNA. Therefore we assume a proportionality between biomass and DNA concentration. Future work should challenge this assumption with an eye toward understanding how age or size structure may affect observed eDNA. In stock assessment terms, such work could inform developing an age- or size- dependent selectivity curve for an eDNA index. We do not pursue such approaches here.
...
We collected eDNA samples during the 2019, 2021, and 2023 U.S.-Canada Integrated Ecosystem \& Acoustic-Trawl Survey for Pacific hake aboard the NOAA Ship *Bell M. Shimada* (all years) and DFO Ship *Sir John Franklin* (2023) conducted between July and September [e.g. @deBlois2020survey]. 
...
Sampling stations were spread across a spatial domain that varied slightly among years (Fig. \ref{fig:station.map.acoustic.lines}) with water sampling occuring between San Francisco, CA and Cape Flattery, WA in 2019, expanding south in 2021 to include Pt. Conception, CA to Cape Flattery, WA, and including a slightly larger northern extent in 2023, spanning Pt. Conception, CA to west of Barkley Sound, BC (Fig. \ref{fig:station.map.acoustic.lines}). This means that the spatial domain where water samples are available varies among years.  We discuss the implications of this varying sampling for developing indices of abundance below.
...
```{r, warning=FALSE,echo=FALSE,message=FALSE}
stations_unique <- read.csv(here("Plots and figures","_QMD_plots","CTD_stations_unique_table.csv")) 

stations_tab <- read.csv(here("Plots and figures","_QMD_plots","CTD_stations_table.csv"))
...
<!-- ```{r fig.station.map, echo=FALSE,fig.cap="\\label{fig:station.map} Locations of water samples collected from CTD stations in each year.", out.width='100%'} -->
<!-- knitr::include_graphics(here("Plots and figures","_QMD_plots","2019-23, CTD locations.pdf")) -->
<!-- ``` -->

```{r fig.station.map.acoustic.lines, echo=FALSE,fig.cap="\\label{fig:station.map.acoustic.lines} Locations of acoustic transect and water samples collected from CTD stations in each year. Only transect lines within the range of water samples are shown", out.width='100%'}
knitr::include_graphics(here("Plots and figures","_QMD_plots","2019-23, CTD locations+acoustics.pdf"))
```

### Defining spatial areas for generating an index

The area sampled by the acoustic-trawl survey varies among years.  In Fig. \ref{fig:station.map.acoustic.lines}, we show the acoustic transect lines sampled during the hake survey and overlay the locations of CTD stations at which water was collected for eDNA sampling. Note that the east-west extent of both the acoustic transects and water sampling stations vary somewhat among years -- transects in 2019 tend to cover more area than transects in 2021 and 2023, particularly in northern California near Cape Mendocino (40.4N).  

To generate a spatial domain for each year, we used a 5km resolution gridded maps to project eDNA concentration. This vector-based grid was developed by Feist et al. [-@feist2021footprints] and uses a custom coordinate reference system that conserves area and distance reasonably well across the west coast of the United States (see also Shelton et al. 2022). Each grid cell has an associated area-weighted mean bottom depth as well. To account for changes in survey area among years, we defined a polygon for each year by including the entirety of acoustic transects from that year and connecting adjacent transects along their eastern-most extent and their western-most extent. All 5km grid cells whose centriods fell within this polygon were included in the projection set. Because eDNA varies with depth as well as geospatially, we made prediction for each grid cell in 50m depth increments up to 500m or the bottom depth of each cell. 

Finally, we defined three areas based on latitude to enable the creation of abundance indices that are comparable across years.  We created a core area between Cape Flattery, WA and San Franscisco Bay, CA that is shared among all years.  In addition, this region has historically contained the majority of hake biomass detected during the hake acoustic survey (hereafter "core area"; Fig. \ref{fig:area.by.year}).  We defined a second area from San Francisco Bay to Pt. Conception (hereafter "south area"), and a small area north of Cape Flattery, WA (hereafter "north area"). Observations of hake DNA are available for all years for the core area, for the south area in 2021 and 2023, and for the north area in 2023 (see Fig. \ref{fig:station.map.acoustic.lines}), therefore we report summaries of hake DNA in each area for the appropriate years. 
...
```{r fig.area.by.year, echo=FALSE,fig.cap="\\label{fig:area.by.year} Areas included in projections", out.width='100%'}
knitr::include_graphics(here("Plots and figures","_QMD_plots","2019-23, projection areas.pdf"))
```


```{r, warning=FALSE,echo=FALSE,message=FALSE}

comb_by_year_tab<- read.csv(file=here("Plots and figures","_QMD_plots","Area_projections.csv"))
...
We first present an overview of water sample level observations (Fig. \ref{fig:raw.hake.DNA}). While this figure is busy, we can see that the spatial distribution of observed eDNA concentration varies substantially among depths and years. Note in addition that there are clearly a few samples in each year that are notably higher concentration than the average.

```{r plot_raw_eDNA ,echo=FALSE, warning=FALSE, fig.cap="\\label{fig:raw.hake.DNA} Raw estimates of hake eDNA concentraion by year (rows) and sampling depth (columns).   Each circle represents estimated eDNA from a single water bottle. Black dots represent samples in which amplification was not detected",fig.width=9,fig.height=9,fig.align='center'}
knitr::include_graphics(here("Plots and figures","_QMD_plots","2019, 2021, 2023, Hake raw copies.pdf"))
...
```{r plot_D_i_pred ,echo=FALSE, warning=FALSE, message=FALSE,fig.cap="\\label{fig:depth.preds.Di.all} Predictions of hake DNA concentration at sampled locations across years (rows) and sampled depths (columns).",fig.height=7,fig.width=11}

knitr::include_graphics(here("Plots and figures","_QMD_plots","p_D_all.png"))
```

```{r plot_D_i_pred_sm ,echo=FALSE, warning=FALSE, message=FALSE,fig.cap="\\label{fig:depth.preds.Di.few} Predictions of hake DNA concentration at sampled locations for two sampling depths (rows) across years (columns).",fig.height=8}
knitr::include_graphics(here("Plots and figures","_QMD_plots","p_D_few_19_23.png"))
```

```{r, echo=FALSE,fig.cap="\\label{fig:depth.preds.smooth} Predictions of Hake DNA concentration from the spatio-temporal model across multiple years (rows) and depths (columns).", fig.height=11,fig.align='left'}
knitr::include_graphics(here("Plots and figures","_QMD_plots","pred_D_3D.png"))
```

```{r, echo=FALSE,message=FALSE,fig.cap="\\label{fig:depth.preds.smooth.few} Predictions of Hake DNA concentration from the spatio-temporal model for two depths (rows) among years.  Note that predictions are made to locations included in any year, and may include spatial regions that do not have observations in a given year (e.g. south of San Francisco Bay in 2019)", fig.height=7,fig.align="left"}

knitr::include_graphics(here("Plots and figures","_QMD_plots","p_D_pred_all.png"))

```

Another way to look at the spatial patterns are along-transect perpendicular to the coastline.  We show predictions of two East-West slices across multiple years (Fig. \ref{fig:transect1}, \ref{fig:transect2}). Note the color scale is not identical among transect figures.

```{r plot_D_trans1 ,echo=FALSE,message=FALSE, warning=FALSE,fig.height=6,fig.width=9,fig.cap="\\label{fig:transect1} Predicted hake DNA concentration (copies per uL) along E-W transect at 37.58 N. Projections are in 50m depth bins.",fig.width=7,out.width="100%"}
# Spatial predictions in east-west transects
knitr::include_graphics(here("Plots and figures","_QMD_plots","p_trans_71.png"))
```

```{r plot_D_trans2 ,echo=FALSE,message=FALSE, warning=FALSE,fig.height=6,fig.width=9,fig.cap="\\label{fig:transect2} Predicted hake DNA concentration (copies per uL) along E-W transect at 40 N. Projections are in 50m depth bins.",fig.width=7,fig.align="left",out.width="100%"}
knitr::include_graphics(here("Plots and figures","_QMD_plots","p_trans_125.png"))
```


These predictions are the combination of a fixed effect of year intercept (Fig. \ref{fig:fixed.intercept}) as well as the smooth as a function of bottom depth (Fig. \ref{fig:depth.smooth}). The year intercepts should not be interpreted as an estimate of overall abundance in each year because the sampling frame changes substantially among years. The depth smooth finds that peak hake abundance corresponds to bottom depths between approximately 100m and 200m (Fig. \ref{fig:depth.smooth}). This corresponds to the continental shelf break and agrees with existing information about hake distribution.

```{r plot_Fixed ,echo=FALSE, warning=FALSE,message=FALSE,fig.cap="\\label{fig:fixed.intercept} Year intercept and 95 percent intervals",fig.width=5,out.width="80%"}
knitr::include_graphics(here("Plots and figures","_QMD_plots","p_fixed_effects.png"))
```

```{r plot_smoothed ,echo=FALSE, warning=FALSE,message=FALSE,fig.cap="\\label{fig:depth.smooth} Marginal effect of depth (p-spline smooth).",fig.width=5,out.width="70%"}
knitr::include_graphics(here("Plots and figures","_QMD_plots","p_smooth_effects2.png"))
```
...
```{r plot_bottle_effects ,echo=FALSE, warning=FALSE,message =FALSE, fig.cap="\\label{fig:bottle.1} Random bottle effects by year and depth",fig.width=5,,out.width="80%"}
# Print the spatial distribution of bottle effects by depth, year, and latitude.
knitr::include_graphics(here("Plots and figures","_QMD_plots","p_tau_depth.png"))
```

Finally, we can look calculate the correlation of the spatial fields among water depths. This is correlation that derives from from the estimated factor weight matrix (see *Statistical Model*) and shows that there are strong estimated correlation among depths and but that generally, the surface samples are at best weakly correlated with depths below 150m or so where the majority of hake and hake DNA is found (Fig. \ref{fig:depth.smooth}).

```{r plot_correlation_spatial ,echo=FALSE,message=FALSE, warning=FALSE,fig.width=8,fig.cap="\\label{fig:corr_D} Estimated correlation among depths for the  spatial effect.",out.width="80%"}
knitr::include_graphics(here("Plots and figures","_QMD_plots","p_corr_plot.png"))
```


We construct the eDNA index by generating predictions to each depth between the surface and 500m in 50m intervals and then summing. We can therefore sum across depths within each 5km grid cell to generate a depth-integrated map of DNA in each year (Fig. \ref{fig:plot_2D_1}). Note that in these figures we make predictions to the entire spatial domain, even if there are no observations near some locations in particular year (e.g. south of San Francisco Bay in 2019). In general, the plots show higher DNA concentrations in 2019 relative to 2021 or 2023 particularly north of Cape Mendocino. 

Differences among years become more obvious when we plot the index as a ratio between years to understand how hake DNA has changed in distribution between years (Fig. \ref{fig:plot_2D_diff}).  Here bright colors indicate areas where estimated DNA concentration changed dramatically between years. For example, the predominance of blue in the panels "ratio_19_21" and "ratio_19_23" indicate areas that are substantially higher in 2019 relative to the other years.  


```{r plot_D_2D_1 ,echo=FALSE, warning=FALSE, message=FALSE, fig.cap="\\label{fig:plot_2D_1} Depth-integrated eDNA index for each year.", out.width="100%",fig.align="left"}
# Spatial predictions
knitr::include_graphics(here("Plots and figures","_QMD_plots","p_D_pred_2D.png"))

```

```{r plot_D_2D_ratio ,echo=FALSE, warning=FALSE, message=FALSE, fig.cap="\\label{fig:plot_2D_diff} The ratio of the depth-integrated eDNA index between all pairs of years for each year. This shows how areas have changed in their hake DNA among years. Each panel shows the relationship between two years. For example, ratio_19_21 is the predictions from 2021 divided by the predictions from 2019.  This means blue cells are areas in which the predictions are larger in 2019 that 2021 (red areas indicate the converse). Grey areas indicate areas that are out of the survey area for one or both of the years.", fig.height=8,fig.align="left",out.width="100%"}
# Spatial predictions
knitr::include_graphics(here("Plots and figures","_QMD_plots","p_D_pred_2D_ratio.png"))
```
...
Following the estimation methods, we summed across the predictions to generate a coastwide index of abundances.  We repeated this approach for 10 different spatial meshes to provide an estimate for an eDNA index for the core and south areas (Fig. \ref{fig:index_1}). We show estimates for the individual model configurations and the consensus index (Fig. \ref{fig:index_1}). Note that all model provide qualitatively the same pattern: a substantial ($\approx 35\%$) decline in DNA concentration between 2019 and 2021 a most further decline between 2021 and 2023.  For 2021 and 2023 we can combine the core and south areas to provide an index covering Pt. Conception, CA to Cape Flattery, WA (Fig. \ref{fig:index_2}). Our models suggest that the total DNA in 2023 in both the core and south areas was roughly equivalent to the DNA in the core area in 2019. 

```{r , warning=FALSE,echo=FALSE,message=FALSE,fig.cap="\\label{fig:index_1} eDNA index calculated for appropriate years for the core and south areas (see also Fig. 2). Colored lines show estimates from individual spatio-temporal models and 95 percent interval. Black lines show the among model average, interquantile range and 90 percent intervals.", out.width='100%'}

knitr::include_graphics(here("Plots and figures","_QMD_plots","eDNA_index_1.png"))
```

```{r , warning=FALSE,echo=FALSE,message=FALSE,fig.cap="\\label{fig:index_2} eDNA index calculated for appropriate years for the core and south areas (see also Fig. 2). Colored lines show estimates from individual spatio-temporal models and 95 percent interval. Points and error bars show the among model average, interquantile range and 90 percent intervals.", out.width='80%'}
knitr::include_graphics(here("Plots and figures","_QMD_plots","eDNA_index_2.png"))
```
...
We start by plotting the qPCR standards (qPCR analyses on DNA of a known concentration) for a single PCR plate individual, all of the plates run within a single year, and then all of the estimated relationships across all years.  For the presence-absence components and for the positive components (Figs. \ref{fig:standard_pres-abs}, \ref{fig:standard_pos}, \ref{fig:standard_pos2} )  
```{r plot_stand_1 ,echo=FALSE, warning=FALSE,fig.cap="\\label{fig:standard_pres-abs} Presence-absence plot for standards.",out.width="100%"}
# Print the presence-absence info for standards
knitr::include_graphics(here("Plots and figures","_QMD_plots","p_bin_stand.png"))

```

```{r plot_stand_2 ,echo=FALSE, warning=FALSE,fig.cap="\\label{fig:standard_pos} Positive observations for standards. Left panel shows observations and fits from a single plate with known DNA concentrations, while the right panels shows the positive observations for all standard plates in a given year (points have been jittered to reduce overlap). ",out.width="100%" }
# Print the presence-absence info for standards
knitr::include_graphics(here("Plots and figures","_QMD_plots","p_pos_stand.png"))
# Print the postive part for standards
```

```{r plot_stand_3 ,echo=FALSE, warning=FALSE,fig.cap="\\label{fig:standard_pos2} Positive observations and best fit lines for standards (all years, all plates).",out.width="100%" }
# Print the presence-absence info for standards
knitr::include_graphics(here("Plots and figures","_QMD_plots","p_pos_stand_year.png"))
# Print the postive part for standards
```
...
We provide a few plots of predictions and observations for pres-abs and positive components (each faceted by water depth) (Figs. \ref{fig:unk_pres-abs}, {fig:unk_pres-abs}). We also present two figures of the relationship between DNA concentration ($D$) and the concentration within a given bottle ($E$; Figs. \ref{fig:D_v_E1}, \ref{fig:D_v_E2}) to illustrate the effect of the bottle random effects and offsets that affect each sampled water bottle.

```{r plot_pres_abs_unk ,echo=FALSE, warning=FALSE,message=FALSE,fig.cap="\\label{fig:unk_pres-abs} Prediction vs. observed, presence-absence component",fig.height=7,out.width="100%"}
# Print the presence-absence info for field samples.
#print(p_bin_unk2)
knitr::include_graphics(here("Plots and figures","_QMD_plots","p_bin_unk_samp.png"))
```

```{r plot_pos_unk ,echo=FALSE, warning=FALSE,message=FALSE,fig.cap="\\label{fig:unk_pres-abs} Prediction vs. observed, positive component. Note the rare outliers; outliers are almost always one bottle of very high concentration that has a replicate bottle that has very low estimated concentration. Lines are loess smoothes. ",fig.height=7, out.width="100%"}
# Print the postive part for field samples.
knitr::include_graphics(here("Plots and figures","_QMD_plots","p_pos_unk_samp.png"))
#print(p_pos_unk)
```

```{r plot_D_v_E ,echo=FALSE, warning=FALSE,fig.height=6,fig.cap="\\label{fig:D_v_E1} Plot of the latent variable D vs. E (the concentration assayed by qPCR) faceted by year (columns) and dilution (rows). Note how the dilution shifts all points down relative to the 1:1 line.",out.width="100%"}
knitr::include_graphics(here("Plots and figures","_QMD_plots","p_Di_Ei_1.png"))
```

```{r plot_D_v_E2 ,echo=FALSE, warning=FALSE,fig.height=9,fi.gcap="\\label{fig:D_v_E2} Plot of the latent variable D vs. E for undiluted samples for years (columns) and water depths (rows).",out.width="100%"}
knitr::include_graphics(here("Plots and figures","_QMD_plots","p_Di_Ei_2.png"))
```

<!-- ```{r plot_bottle_2 ,echo=FALSE, warning=FALSE,fig.height=9,fi.gcap="\\label{fig:bottle_2} Plot of the latent variable D vs. E for undiluted samples.  Faceted by depth.",out.width="100%"} -->
<!-- knitr::include_graphics(here("Plots and figures","_QMD_plots","p_bottle_2.png")) -->
<!-- ``` -->

<!-- ```{r plot_L_factor ,echo=FALSE, warning=FALSE,fig.height=4, fig.cap="\\label{fig:factors} Estimated Factors as a function of depth.",out.width="100%"} -->
<!-- knitr::include_graphics(here("Plots and figures","_QMD_plots","p_L_factor.png")) -->
<!-- ``` -->
