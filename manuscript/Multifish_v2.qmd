...
What we need to have in Methods:

-   Brief introduction of the model concept of quantitative MB \[Done\]
-   The sampling site, number of samples, sampling protocol \[partially done? maybe Ryan can add more?\]
-   Construction of standards and mock community \[waiting for Megan's details\]
-   DNA extraction
-   qPCR for hake (primer, concentration, PCR conditions, etc) \[waiting for Megan's details\]
-   MB (primers, concentration, PCR conditions, etc) \[waiting for Megan's details\]
-   Bioinformatics, annotation \[waiting for Megan's + Pedro's details\]
-   Model specificity (stan condicitions) and spatial model conditions \[Gled + Owen\]

------------------------------------------------------------------------

We developed and joined three distinct datasets (qPCR targeting hake, metabarcoding sequencing reads for a mock community with known concentrations, and metabarcoding sequencing reads for environmental samples with unknown concentrations) into a joint Bayesian model following the formulation in \cite{guri_2024}. In brief, we calibrated metabarcoding data, correcting raw species observations to their initial proportions using a community of known composition \cite{shelton_2022} . We then used the absolute quantity of one species to expand the initial estimated proportions into absolute abundance estimates for each species (\cite{guri_2024} and see also Allan et al. 2023) in each of \[347\] 2L water samples collected during a 2019 NOAA cruise (see Ramon-Laca et al. 2021; Shelton et al. 2022). The details for creating each dataset is explained below. Subsequently the estimated eDNA quantities were spatially smoothed using sdmTMB package (an approximation of Gaussian Process models) for generating species distribution maps.
...
### Construction of standard samples (qPCR) and mock community (metabarcoding)

#### How were the qPCR standards constructed?

The DNA from tissue samples of Pacific hake was extracted using the DNeasy Blood & Tissue Kit (Qiagen GmbH, Hilden Germany). To create standard samples, we amplified the xxx gene region using xxx primer with the thermocycler program xxx. Subsequently the samples underwent a series of dilutions to achieve final concentrations ranging from $10^{0}$ – $10^{5}$ copies/µL.

#### How were the mock community constructed?

To calibrate metabarcoding observations and account for amplification bias (Gold et al., 2023; Shelton et al., 2023a), we generated mock communities comprising a total of xxx fish species. The DNA extracts obtained from fin or other tissues of species (see supplementary information xxx), was measured targeting 12S rRNA genene using MiFish-U universal primer Miya_2015 and evagreen probe in QX200 ddPCR system (Bio-Rad). Each reaction used 5 μL of DNA template from genomic DNA of each species, 10 μL of ddPCR Supermix with no dUTP (Bio-Rad), 900 nM of final concentration of forward and reverse primers (Table 1), and 250 nM final concentration of TaqMan probe for each assay (cod and herring respectively). The thermocycler reactions were run in C1000 Touch Thermal Cycler with 96-Deep Well Reaction Module (Bio-Rad) using the PCR program as follows: 10 min at 95°C for enzyme activation, followed by 44 cycles of denaturation for 1 min at 95°C and primer annealing and elongation for 2 min at 55.6° C, with a ramp rate of 2° C per s, followed by 10 min at 98° C and stored at 4° C. Subesquently, the DNA from each species was pooled on equimolar and non-equimolar concentrations to form an even and a skewed mock community respectively. PCR products were diluted in 10-fold steps to achieve a final concentration of \~ $10^3$ copies/µL.

A total of eight mock communities (4 even and 4 skewed) samples were amplified and sequenced following the same workflow and protocol as the environmental samples. Only 13 species in the mock community including Pacific herring (*Clupea pallasii*), northern anchovy (*Engraulis mordax*), northern smoothtongue (*Leuroglossus stilbius*), Dover sole (*Microstomus pacificus*), Pacific sardine (*Sardinops sagax*), Pacific chub mackerel (*Scomber japonicus*), widow rockfish (*Sebastes entomelas*), northern lampfish (*Stenobrachius leucopsarus*), California barracudina (Tactostoma macropus), plainfin midshipman (*Tarletonbeania crenularis*), eulachon (*Thaleichthys pacificus*), Pacific jack mackerel (*Trachurus symmetricus*), and Pacific hake (*Merluccius productus*) were used in the downstream workflows and analysis.

### DNA extraction

DNA extraction of water samples followed the manufacturer's protocol using the DNeasy PowerWater Sterivex Kit (Qiagen GmbH, Hilden Germany). Negative controls such as field water and air blanks, laboratory blanks, and PCR blanks were taken throughout the workflow. The filtered eDNA from the environmental samples were used in qPCR analysis and metabarcoding sequencing downstream.

### qPCR quantification of Pacific hake

Quantification of Pacific hake from environmental samples were analysed in duplicates using the same primer-probe as the standards on a QuanStudio 6 (Applied Biosystems). The thermocycler protocol consisted of 10 min at 95 °C for denaturation followed by the cycling stage of 40 cycles of 15 sec at 95 °C and 1 min at 58 °C. All thermocycler reactions were run in 20 µL volume consisting of 10 µL of TaqMan Environmental Master Mix 2.0, 1 µL of each primer (forward and reverse, 10 nM concentration each; see Table 1), 0.5 µL of probe (10nM concentration), 3 µL of dH20 and 2 µL of DNA template. All the runs included an internal positive control (IPC) of the reaction to account for PCR inhibition. Any delay of more than 0.5 cycles from the IPC at the non-template controls of the PCR was considered inhibition. Negative controls were run together with the samples.

### Metabarcoding and Sequence Annotation

In total, 347 samples divided into xxx libraries, including eDNA samples (n = xxx), PCR blanks (n = xxx), positive controls (n = 8), extraction blanks (n = xxx), fieldwork water and air blank (n = xxx and xxx respectively), were amplified using MiFish-U universal primer Miya_2015. We used two-step PCR protocol and followed a thermocycler program of xxx. Libraries were sequenced on Illumina sequencer using the xxx-sequencing chip. DADA2 was used to denoise sequences, remove chimeras, and generate an amplicon sequence variant (ASV). Sequences were annotated using LCA algorithms and confirmed via BLAST function against NCBI database (access: August 2024). Samples with fewer total number of reads than 1000 were removed from downstream analysis as indicator of poor quality samples.

### Statistical analysis (Stan & spatial model)

We estimated amplification efficiencies of the listed species from mock community metabarcoding run as described in \cite{shelton_2022} using the Stan language as implemented in R (package: Rstan) running 4 MCMC chains with 5000 warmup and 5000 sampling iterations each (see Supplementary math & code). We then used the amplification efficiencies for each species to estimate the initial proportions of each species in environmental samples and the absolute quantity of Pacific hake (from qPCR) to expand the estimated proportions into absolute eDNA estimates (copies/μL) for each species in each sample as described om \cite{guri_2024} and Allan et al. 2023 in a joint model. The joint model was implemented Stan running 4 MCMC chains with 1000 warmup and 2000 sampling iterations each (see Supplementary math & code). The posterior of both model predictions were diagnosed using $\hat{R}$ statistics (Gelman and Rubin, 1992) and considered convergence for $\hat{R}$ values less than 1.1 and effective sample size (ESS) greater than 200 for all parameters.
