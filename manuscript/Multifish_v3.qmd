...
[Maybe one more sentence about methods / joint model above here, to ease transition into Results]
...
[To be shortened; also emphasize that these are common species about which relatively little is known]
...
We developed and joined three distinct datasets (qPCR targeting hake (*Merluccius productus*) as a reference species [@shelton2022], metabarcoding sequencing reads (12s mtDNA) for a mock community of known composition, and metabarcoding sequencing reads (12s mtDNA) for environmental samples with unknown concentrations) into a joint Bayesian model following the formulation in [@guri2024a]. In brief, we (1) calibrated metabarcoding data, correcting raw species observations to their initial proportions using the mock community [@shelton2023], (2) used the absolute quantity of hake eDNA to expand the initial estimated proportions into absolute abundance estimates for each species  [@guri2024a; and see also @allan2023] in each of \[347\] 2L water samples collected during a 2019 NOAA cruise [see @ramon-laca2021; and @shelton2022], and (3) spatially smoothed the estimated eDNA quantities to generate species distributions.
