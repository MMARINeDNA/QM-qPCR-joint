...
```{r,eval=F}
# fitted model object
jm_out <- read_rds(here('data','Zenodo','Joint_mod_output.rds'))
stan_data <- read_rds(here('data','Zenodo','Joint_mod_data_input.rds'))
...
# write
write_rds(copies_fitted,here('data','Zenodo','Log_D_est.rds'))
rm(list=ls())
```

```{r}
dat <- read_rds(here('data','Zenodo','Log_D_est.rds'))
glimpse(dat)
...
# save
write_rds(all_preds,here('data','Zenodo','Log_D_est_smoothed.rds'))
```
