...
sp_maps <- cowplot::plot_grid(plotlist=sp_dat_maps,nrow=3)
...
What about a version of the data where we impose absence as <0.5 estimated copies

```{r}
dat_zeroes <- dat_utm %>% 
  mutate(depth_num=depth) %>% 
  mutate(conc=exp(mean)) %>% 
  mutate(conc=ifelse(conc<0.5,0,conc))
# look at the 'raw' integrated data
...
meshes <- purrr::map(seq(5,80,by=5),function(co){
...
anch1 <-  sdmTMB(
  ...
  formula = conc~0+dist_shelf_scaled+dist_shelf_scaled2+depth_cat,
  ...
  mesh=make_mesh(dat_anchovy,c('utm.lon.km','utm.lat.km'),mesh=meshes[[4]]),
  family = tweedie(),
  spatial = "on"
)
sanity(anch1)
...
anchovy_test <- purrr::map(meshes,function(x){
  spatial_only_model(df=dat_zeroes,f='conc~0+dist_shelf_scaled+dist_shelf_scaled2+depth_cat',spp = "Engraulis mordax",mesh = x)
})
...
Looks like Mesh 4 is the best (at least for anchovy), with 124 vertices.
Eulachon and herring had a tougher time fitting with this one:

```{r}
eul_test <- purrr::map(meshes,function(x){
  spatial_only_model(df=dat_zeroes,f='conc~0+dist_shelf_scaled+dist_shelf_scaled2+depth_cat',spp = "Thaleichthys pacificus",mesh = x)
})
...
```
For eulachon, the mesh with only 36 vertices worked best (Mesh 16)

```{r}
clu_test <- purrr::map(meshes,function(x){
  spatial_only_model(df=dat_zeroes,f='conc~0+dist_shelf_scaled+dist_shelf_scaled2+depth_cat',spp = "Clupea pallasii",mesh = x)
})
...
```
For herring, we can use the mesh with 64 vertices (Mesh 8)
...
anchovy_m <- spatial_only_model(df=dat_zeroes,f='conc~0+dist_shelf_scaled+dist_shelf_scaled2+depth_cat',spp = "Engraulis mordax",mesh=meshes[[4]])

hake_m <- spatial_only_model(df=dat_zeroes,f='conc~0+dist_shelf_scaled+dist_shelf_scaled2+depth_cat',spp = "Merluccius productus",mesh=meshes[[4]])

sardine_m <- spatial_only_model(df=dat_zeroes,f='conc~0+dist_shelf_scaled+dist_shelf_scaled2+depth_cat',spp = "Sardinops sagax",mesh=meshes[[4]])

mackerel_m <- spatial_only_model(df=dat_zeroes,f='conc~0+dist_shelf_scaled+dist_shelf_scaled2+depth_cat',spp = "Trachurus symmetricus",mesh=meshes[[4]])

eulachon_m <- spatial_only_model(df=dat_zeroes,f='conc~0+dist_shelf_scaled+dist_shelf_scaled2+depth_cat',spp = "Thaleichthys pacificus",mesh=meshes[[16]])

herring_m <- spatial_only_model(df=dat_zeroes,f='conc~0+dist_shelf_scaled+dist_shelf_scaled2+depth_cat',spp = "Clupea pallasii",mesh=meshes[[8]])
...
make_pred_map(anchovy_m)+ggtitle("Anchovy")
make_pred_map(hake_m)+ggtitle("Hake")
make_pred_map(sardine_m)+ggtitle("Sardine")
make_pred_map(mackerel_m)+ggtitle("Mackerel")
make_pred_map(eulachon_m)+ggtitle("Eulachon")
make_pred_map(herring_m)+ggtitle("Herring")
```
