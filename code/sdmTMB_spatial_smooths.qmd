...
dat_anchovy <- dat_utm %>%
  filter(species=="Engraulis mordax",depth_cat==0) %>%
...
anch1 <-  sdmTMB(
  ...
  formula = copies~log_bathy_scaled+log_bathy_scaled2,
  mesh=make_mesh(dat_anchovy,c('utm.lon.km','utm.lat.km'),mesh=mesh3),
...
anch3 <-  sdmTMB(
  ...
  mesh=make_mesh(dat_anchovy_int,c('utm.lon.km','utm.lat.km'),mesh=mesh3),
  family = tweedie(link='log'),
  spatial = "on",
  anisotropy = F
)
...
anch1_preds_ch <- predict(anch1,newdata=ch_pred_df) %>%
...
```{r}
# depth-integrated hake
dat_hake_int <- dat_utm %>%
  mutate(copies=exp(mean)) %>%
...
hake1 <-  sdmTMB(
  data = dat_hake_int,
  formula = tot~log_dist+log_dist2,
  mesh=make_mesh(dat_hake_int,c('utm.lon.km','utm.lat.km'),mesh=mesh3),
  family = tweedie(link='log'),
  spatial = "on",
  anisotropy = F
)
hake1_preds_ch <- predict(hake1,newdata=ch_pred_df %>% filter(depth_cat==0)) %>% 
  left_join(ch_pred,by=join_by(grID)) %>%
  filter(dist_shore>5) %>%
  st_as_sf()
hake1_preds_ch_plot <- ggplot()+
  geom_sf(data=coaststates,fill='gray80')+
  geom_sf(data=hake1_preds_ch,aes(fill=est),color=NA)+
  # geom_sf(data=dat_sf,color='red',size=0.5,shape=4)+
  scale_fill_viridis()+
  xlim(dat_bbox[1],dat_bbox[3])+ylim(dat_bbox[2],dat_bbox[4])+
  labs(fill="Log Total Copies",title="Merluccius productus",x="")
hake1_preds_ch_plot
```
...
anch3_preds_ch_plot <- ggplot()+
  geom_sf(data=coaststates,fill='gray80')+
  geom_sf(data=anch3_preds_ch,aes(fill=est),color=NA)+
  # geom_sf(data=dat_sf,color='red',size=0.5,shape=4)+
  scale_fill_viridis()+
  xlim(dat_bbox[1],dat_bbox[3])+ylim(dat_bbox[2],dat_bbox[4])+
  labs(fill="Log Total Copies",title="Engraulis mordax",x="")
