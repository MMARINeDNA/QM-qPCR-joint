...
// from qPCR estimates, alphas, and etas we can calculate sample-specific mu
 ...
 {// local variables for making reference species vectors
      matrix[N_species,N_obs_mb_samp] prob_samp_t; // estimates of read counts, in log space
      matrix[N_species,N_obs_mock] prob_mock_t; // estimates of read counts, in log space
...
model{
  ...
  // QM Likelihoods
  for(i in 1:N_obs_mock){
    mock_data[i,]   ~  dirichlet_multinomial(transpose(mu_mock[i,]),alpha_0); // Multinomial sampling of mu (proportions in mocks)
  }

  // print("2:",target());
  for(i in 1:N_obs_mb_samp){
    sample_data[i,] ~  dirichlet_multinomial(transpose(mu_samp[i,]),alpha_0); // Multinomial sampling of mu (proportions in field samples)
  }
 //  print("3:",target());
