...
qPCR_mb_link <- prepare_stan_qPCR_mb_join(input_metabarcoding_data=mfu,
                                          unk_formatted = formatted_qPCR_data$tube_dat,
                                          link_species = "Merluccius productus")
...
fit_summ <- get_param_ests(jointMod)
```
...
plot_obs_pred_qm(fit_summ,stan_data_joint)
```
...
reads_fitted <- reads_fitted %>% 
  mutate(species=as.factor(unique(mfu$species))[taxa]) %>% 
  left_join(mfu_META,by=join_by(rep==sampleidx))
...
samps <- read_csv(here('code','tmpU.csv'),skip = 25)
sampslong <- samps %>% 
...
plot_trace(sampslong,"alpha")
plot_trace(sampslong,"log_D")
